...
type ApiGasPricePriorityRequest struct {
	ctx context.Context
	ApiService *GasAPIService
	priority string
}

func (r ApiGasPricePriorityRequest) Execute() (string, *http.Response, error) {
	return r.ApiService.GasPricePriorityExecute(r)
}
...
Get estimated gas price with priority filter based on historical data
 ...
 @return ApiGasPricePriorityRequest
*/
...
func (a *GasAPIService) GasPricePriorityExecute(r ApiGasPricePriorityRequest) (string, *http.Response, error) {
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "GasAPIService.GasPricePriority")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/gas/price/{priority}"
	localVarPath = strings.Replace(localVarPath, "{"+"priority"+"}", url.PathEscape(parameterValueToString(r.priority, "priority")), -1)
