...
func (r ApiValidatorVotesRequest) Execute() ([]ResponsesVote, *http.Response, error) {
	return r.ApiService.ValidatorVotesExecute(r)
}
...
func (a *ValidatorAPIService) ValidatorVotesExecute(r ApiValidatorVotesRequest) ([]ResponsesVote, *http.Response, error) {
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ValidatorAPIService.ValidatorVotes")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/validators/{id}/votes"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
